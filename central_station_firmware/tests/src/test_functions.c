...
void clean_previous_readings_list(ambient_info_t previous_readings[LENGTH_PREVIOUS_READINGS_ARRAY]) {
    for (int i = 0; i < LENGTH_PREVIOUS_READINGS_ARRAY; i++) {
        ...
        previous_readings[i].alcohol_concentration = 0.0;
...
void create_rising_temperature_hazard(ambient_info_t previous_readings[LENGTH_PREVIOUS_READINGS_ARRAY]) {
  ...
  for (int i = 1; i < LENGTH_PREVIOUS_READINGS_ARRAY; i++) {
    ...
    previous_readings[i].alcohol_concentration =
        previous_readings[i - 1].alcohol_concentration +
        (ALCOHOL_WORSENING_MARGIN - 0.05);
...
/**
 * @brief Construct a list of prior readings that presents a hazard due to the
 * alcohol concentration continuously worsening.
 * 
 * @param previous_readings the previous readings list where the test data will be stored.
 */
void create_worsening_alcohol_hazard(ambient_info_t previous_readings[LENGTH_PREVIOUS_READINGS_ARRAY]) {
  ambient_info_t initial_values = {
    .temperature = -2.4,
    .humidity = 13.7,
    .light_intensity = 3.0,
